...
func TestHandleMessage(t *testing.T) {
	...
	testActiveRooms["businessBot-room1"] = "12345"
	testActiveRooms["businessBot-room2"] = "54321"
	bot := new(models.Bot)
	...
	tests := []struct {
	...
	}{
		{
			...
			[]string{},
			[]string{"businessBot-room1", "businessBot-room2"},
			"Message from action",
			"Message from action",
			false,
		},
...
func Test_captureGroups(t *testing.T) {
	...
	testMessage1 := models.Message{
		Service:      models.MsgServiceChat,
		Input:        "deploy businessBot@v0.0.1 to qa",
		Attributes:   make(map[string]string),
		BotMentioned: true,
		Vars:         make(map[string]string),
	}
	...
	tests := []struct {
		...
		{"Capture groups, respond", args{message: testMessage1, rules: testRules1, bot: testBot}, "deploying businessBot at version v0.0.1 to environment qa"},
