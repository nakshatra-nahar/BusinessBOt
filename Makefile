...
BUILD_LDFLAGS := -s -w
BUILD_LDFLAGS += -X github.com/target/businessBot/version.Version=${VERSION}
GOLANGCI_LINT_VERSION := "v1.60.1"
PACKAGES := $(shell go list ./... | grep -v /config-example/)
PLATFORM := "linux/amd64,linux/arm64"

DOCKER_IMAGE ?= "target/businessBot"
...
.PHONY: build
build: clean
	@echo "Building businessBot binary to './businessBot'"
	@go build -a -ldflags '$(BUILD_LDFLAGS)' -o $(PWD)/businessBot ./cmd/businessBot
...
.PHONY: run
run: build
	@echo "Starting businessBot"
	./businessBot

.PHONY: run-docker
run-docker: docker 
	@echo "Starting businessBot docker image"
	@docker run -it --rm --name mybusinessBot -v "$$PWD"/config:/config --env-file .env $(DOCKER_IMAGE):latest /businessBot
